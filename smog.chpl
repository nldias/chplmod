...
// -------------------------------------------------------------------
// -->smog: smoothes a raw ogive Or with a windows of half-width ow
...
proc smog(
   ...
   const ref aow: [] int,     // half-length of the smoothing window
   const ref aOr: [] real,    // the raw ogive
   const in unbiased: bool =true, // to reduce, or not, the bias in Os
   ref aGs: [] real,          // the smoothed spectrum
   ref aOs: [] real,          // the smoothed ogive
   ref cor: correction        // the correction
) { 
   assert(aow.rank == 1);     // make sure arrays are 1D
   assert(aOs.rank == 1);
   assert(aGs.rank == 1);
   const M = aOr.size - 1;    // check their size
   assert(cor.M == M);        // verify if cor has the right size
   assert(aGs.size == M+1);
   assert(aow.size == M+1);
   ...
   for k in 2..M-2 do {       // smoothing loop
      Os[k] = (+ reduce Or[k-ow[k]..k+ow[k]])/(2*ow[k]+1);
   }
   Os[M-1] = Or[M-1];
...
// --------------------------------------------------------------------
   ...
   if unbiased then {
      unbias(sign,deltaf,ow,Gs,Os,cor);
   }
}
// -------------------------------------------------------------------
