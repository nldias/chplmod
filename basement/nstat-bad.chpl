...
proc steep(
   ...
   const in backtr = 0.5           // back trancking line search parameter,
                                   // in [0.2,0.8]
   ) : (real,real,real) where ( ax.rank == 1 && ay.rank == 1 && ap.rank == 1 && asigp.rank == 1 && acp.rank == 2) {
   const maxiter = 10000;
   const m = ax.size;
   const n = ap.size;
   assert (ay.size == m);
...
// -----------------------------------------------------------------------------   
   ...
   var yhat_o: [1..m] real;        // the older one
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------   
   while eps >= epsilon do {
      if iiter > maxiter then {
         halt("nstat-->steep: I have exceeded %i iterations",maxiter);
      }
//      writeln("dely = ",dely[1..10]);
//      writeln("p = ",p);
      chi2p_o = chi2p;
      yhat_o = yhat ;
      chi2p = chi2(p);
...
//         writef("       t: %8.2er   chi2a: %8.2er   chi2b: %8.2er\n",t,chi2a,chi2b);
      ...
      hh = t*gradchi2;
      eps = sqrt(dot_vtv(hh,hh));  // stop criterion
      writeln("eps = ",eps);
      p = p - hh ;                 // update parameters
      iiter += 1;                  // increment iteration
   }
// -----------------------------------------------------------------------------
// error statistics
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------   
   proc simplejacob() {
      ...
      for k in 1..n do {
         ...
         newp[k] += delp[k];
         func(x,newp,yplus);
         J[1..m,k] = (yplus[1..m] - yhat[1..m])/delp[k];
      }
      // for i in 1..10 do {
      //    writeln(J[i,1..2]);
      // }
