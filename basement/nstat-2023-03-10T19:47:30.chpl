...
// -----------------------------------------------------------------------------
      dot_mt_diagm(J,w,maux_nm);
      dot_mv(maux_nm,dely,vaux_n);
// -----------------------------------------------------------------------------
// calculates the LHS of the linear system
// -----------------------------------------------------------------------------
      dot_mm(maux_nm,J,maux_nn);
      gauss(maux_nn,vaux_n,hh);
// -----------------------------------------------------------------------------
// update, etc.
// -----------------------------------------------------------------------------      
      p = p - hh ;
...
proc levmar(
   ...
   const in epsilon_r = 1.0e-1,    // rho criterion
   const in broyden = false        // use Broyden update?
   ) : (real,real,real)
   where ( ax.rank == 1 && ay.rank == 1 && ap.rank == 1
        && asigp.rank == 1 && acp.rank == 2) {
...
// -----------------------------------------------------------------------------   
   ...
   var lamb = 1.0e-2;              // Levenberg-Marquardt's lambda
// -----------------------------------------------------------------------------
// local array variables
// -----------------------------------------------------------------------------   
   var J: [1..m,1..n] real;        // the jacobian matrix
   ...
   var vaux_n2: [1..n] real;       // aux n-vector
   var maux_mn: [1..m,1..n] real;  // aux (m,n)-matrix: maux_nm == J'W
   var
...
// -----------------------------------------------------------------------------
// recalculate the Jacobian matrix?  note that chi2p_o and chi2p have already
// been calculated in the previous iteration, so:
// if p has not changed (chi2p == chi2p_o), p and yhat do not need updating and 
// neither does J!
// -----------------------------------------------------------------------------
      if !broyden then {
         if chip2 != chip2p_o then {    
            func(x,p,yhat);        // if p has changed, update yhat
            dely = yhat - y;       // [hat{y}- y]
            simplejacob();         // new p -> new Jacobian matrix
// -----------------------------------------------------------------------------
// calculate the RHS of the LM linear system
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
            ...
            mdiag(maux_nn,D);                // its diagonal as a matrix
         }
      }
      else {
// -----------------------------------------------------------------------------
...
// iteration, so:
// a) if p has not changed (chi2p == chi2p_o), p and yhat do not need updating 
//    and neither does J!
// b) otherise, update yhat
// -----------------------------------------------------------------------------         
         if chip2p != chi2p_o then {    
            if mod(iiter,2*n) == 0 || chi2p > chi2p_o then {
               func(x,p,yhat);     // if p has changed, update yhat
               dely = yhat - y;    // [hat{y}- y]
               simplejacob();      // new p -> new Jacobian matrix
               ...
               yhat_o = yhat ;     // 1st save old yhat for Broyden
               func(x,p,yhat);     // new p -> new Jacobian matrix     
               dely = yhat - y;    // [hat{y}- y]
               Broyden();          // finally, Broyden
            }
// -----------------------------------------------------------------------------
// calculate the RHS of the linear system
// -----------------------------------------------------------------------------
            dot_mt_diagm(J,w,maux_nm);       // maux_nn == J'W
            dot_mv(maux_nm,dely,vaux_n);     // J'W[hat{y} - y]
// -----------------------------------------------------------------------------
// calculate (almost all of) the LHS of the linear system
// -----------------------------------------------------------------------------
            dot_mm(maux_nm,J,maux_nn);       // maux_nn == J'WJ
            mdiag(maux_nn,D);                // its diagonal as a matrix
         }
...
// -----------------------------------------------------------------------------
// lambda-dependency starts only here!
// -----------------------------------------------------------------------------      
      lambD = lamb*D;              // lambda always changes, but not D
      A_nn = maux_nn + lambD;      // J'WJ + lamb D
      gauss(A_nn,vaux_n,hh);       // solve for hh
// -----------------------------------------------------------------------------
// to calculate rho, sum h'lambda D h to vaux_n
// -----------------------------------------------------------------------------
      dot_mv(lambD,hh,vaux_n1);    // lamb D h
      dot_vtv(hh,vaux_n1,vaux_n2); // h' lamb D h
      vaux_n2 += vaux_n;           // h' lamb D h + J'W[yhat(p) - y]
      var rhoden = sqrt(vtv(vaux_n2,vaux_n2););     // | above |
// -----------------------------------------------------------------------------
// what should I do with the updates?
// -----------------------------------------------------------------------------      
      chi2p_o = chi2(p);           // the old figure of merit
      pa = p - hh ;                // do not update p yet!
      chi2p = chi2(pa);            // the new figure of merit
      rho = (chi2p_o - chi2p)/rhoden;
      if rho > epsilon_r then {
         p = pa ;                  // accept the update
         lamb *= 0.1;              // decrease lamb
         lamb = max(lamb,1.0e-7);  // but limit it
      }
      else {                       // do not update p!
         lamb /= 0.1;              // increase lamb
         lamb = min(lamb,1.0ey);   // but limit it
