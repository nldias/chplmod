...
use ada;
use Math;
proc stat1(const ref x: [] real): real {
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
proc median(ref ax: [] ?at): at where ax.rank == 1 {
   var n = ax.size;
...
proc nanmedian(ref ax: [] ?at): at where ax.rank == 1 {
...
// -----------------------------------------------------------------------------
proc quartiles(
   in ax: [domain(1)] real                   // we need to make a copy!
   ): (real,real,real)  { // where ax.rank == 1
   var n = ax.size;
   if n < 3 then {
...
// ----------------------------------------------------------------------------
proc reglin(const ref x: [] real,const ref y: [] real): 3*real {
   assert (x.rank == 1);
   assert (y.rank == 1);
   var n = x.size;
   if n == 0 then {
...
// v. 32, p. 2088-2094
// -----------------------------------------------------------------------------
proc performance(const ref O: [] real, const ref P: [] real):
...
// ------------------------------------------------------------------------------
proc steep(
   ref x: vec,          // ind variables (used as arg to func) (m x 1)
   ref y: vec,          // data to be fit by func(x,p,y) (m x 1)
   ref aw: [] real,     // array, *not matrix*, of weights (m x 1)
...
// -----------------------------------------------------------------------------
// reindexing: vec's are in-place
// -----------------------------------------------------------------------------   
   x.reindex(1..m);
   y.reindex(1..m);
   p.reindex(1..n);
// -----------------------------------------------------------------------------
// reindexing: arrays by ref
// -----------------------------------------------------------------------------   
...
// -----------------------------------------------------------------------------      
      ...
      var pa = p - t*gradchi2;                    // check two chi^2s
                                                  // pa is a vec
...
proc gnewton(
   ...
   ref aw: [] real,           // array, *not matrix*, of weights
   ref p: vec,                     // initial guess of parameter values  (n x 1)
                                   // returns the estimated parameters
   ref asigp: [] real,             // standard  errors of the parameters
   ref acp: [] real,               // parameter covariance matrix
   ...
   const m = x.size;               // number of data points
...
// -----------------------------------------------------------------------------
   while eps >= epsilon do {
      if iiter > maxiter then {
         writef("nstat-->newton: I have exceeded %i iterations\n",maxiter);
         return(-1.0,-1.0,-1.0);
      }
// -----------------------------------------------------------------------------
// recalculate the Jacobian
...
proc levmar(
   ...
   ref asigp: [] real,             // standard  errors of the parameters
   ref acp: [] real,               // parameter covariance matrix
   const func: proc(const ref ax: vec,  // the independent variables
                    const ref ap: vec,  // the parameters
...
// =============================================================================   
   ...
   const m = x.size;               // the number of points
   const n = p.size;               // the number of parameters
   assert (y.size == m);
   assert (aw.size == m);
   assert (asigp.size == n);
...
// ------------------------------------------------------------------------------
// --> levmar2: nonlinear least squares by curve fitting with the
// Levenberg-Marquard method. Here x is a mat.
// ------------------------------------------------------------------------------
proc levmar2(
   ref x: mat,          // independent variables (used as arg to func) (m x 1)
   ref y: vec,          // data to be fit by func(x,p) (m x 1)
   ...
   const func: proc(const ref ax: mat,  // the independent variables
                    const ref ap: vec,  // the parameters
                    ref yhat: vec),     // in the sim model call func(ax,ap,yhat)
   const in epsilon_p = 1.0e-6          // stop criterion
   ) : (real,real,real)
...
// =============================================================================   
   const epsilon_r = 1.0e-3;       // rho criterion
   const maxiter = 100000;         // the maximum number of iterations
   const m = x.shape(0);               // the number of points
   const l = x.shape(1);
   const n = p.size;               // the number of parameters
   assert (y.size == m);
   assert (w.size == m);
