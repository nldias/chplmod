...
record vec {
   ...
   operator =(ref lhs: vec, const in rhs: [] real) where rhs.rank == 1 {
      lhs.dom = rhs.domain;
      lhs.arr = rhs;
   }
// -------------------------------------------------------------------
...
record mat {
   ...
   // }
   // iter ref these(                      // iterate over a range         @\label{lin:these-range}@
   //    const in rai: range(int),
   //    const in j: int
   //    ) ref {
   //    for i in rai do {
   //       yield arr[i-v0delta,j];
   //    }
   // }
// -------------------------------------------------------------------
// operator overloading: *all* of the subsequent operators return
...
proc tovec(const in x: [] real): vec where x.rank == 1 {
